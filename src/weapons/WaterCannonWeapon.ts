...
export class WaterCannonWeapon extends BaseWeapon {
  ...
  fire(dino: IDino, projectiles: ProjectileArray, effects: WeaponEffects = {}): void {
    ...
    if (this.assets && this.assets.pewSound) {
      const sound = this.assets.pewSound.cloneNode() as HTMLAudioElement;
   ...
   */
  checkCollisions(enemies: IEnemy[], onHit: (enemy: IEnemy, details?: HitDetails) => void): void {
    this.blasts.forEach(blast => {
