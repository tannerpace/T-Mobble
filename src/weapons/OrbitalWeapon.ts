...
export class OrbitalWeapon extends BaseWeapon {
   ...
   */
  private getOrbColor(index: number): string {
    const colors = [
      '#66FFFF', // Cyan
      '#FF66FF', // Magenta
      '#FFFF66', // Yellow
   ...
   */
  private findTarget(enemies: IEnemy[]): Orb | null {
    ...
    for (const enemy of enemies) {
      // Skip phased ghosts
      if (enemy.canDamagePlayer && !enemy.canDamagePlayer()) continue;
      // Skip already hit enemies for this orb
      if (availableOrb.hitEnemies.has(enemy)) continue;
  ...
  checkCollisions(enemies: IEnemy[], onHit: (enemy: IEnemy) => void): void {
    ...
    this.orbs.forEach(orb => {
      ...
      enemies.forEach(enemy => {
        // Skip phased ghosts
        if (enemy.canDamagePlayer && !enemy.canDamagePlayer()) return;
